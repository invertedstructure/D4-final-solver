...
def _svr_run_once_computeonly_hard(ss=None):
    ...
    # strict
    try:
        Hd = _hard_co_mm2(H2, d3)
        mC, nC = _hard_co_shape(C3)
        CxorI = _hard_co_xor(C3, _hard_co_eye(nC)) if (mC==nC) else C3
        R3 = _hard_co_xor(Hd, CxorI)
        zero_cols = _hard_co_allzero_cols(R3)
        strict_payload = {"policy_tag": "strict(k=3)",
                          "results": {"k3": {"eq": bool(all(zero_cols)) if R3 else False},
                                      "selected_cols": [1]*(_hard_co_shape(d3)[1]),
                                      "mismatch_cols_selected": [j for j,z in enumerate(zero_cols) if not z]}}
    except Exception as e:
        strict_payload = {"policy_tag": "strict(k=3)",
                          "results":{"k3":{"eq": None}}, "na_reason_code": f"SHAPE_MISMATCH:{str(e)}"}
    ...
    mC, nC = _hard_co_shape(C3)
    if not (mC==nC):
        auto_payload = {"policy_tag": "projected(columns@k=3,auto)",
        ...
        if sum(lanes)==0:
            ...
            try:
                R3 = _hard_co_xor(_hard_co_mm2(H2,d3), _hard_co_xor(C3, _hard_co_eye(nC)))
                eq = _hard_co_masked_allzero_cols(R3, lanes)
                mism = []
                mR,nR = _hard_co_shape(R3)
                for j in range(nR):
                    if lanes[j]==1:
                        for i in range(mR):
                            if R3[i][j]==1:
                                mism.append(j); break
                auto_payload = {"policy_tag": "projected(columns@k=3,auto)",
                                "results":{"k3":{"eq": bool(eq)}, "selected_cols": lanes, "mismatch_cols_selected": mism}}
            except Exception as e:
                auto_payload = {"policy_tag": "projected(columns@k=3,auto)",
                                "results":{"k3":{"eq": None}}, "na_reason_code": f"SHAPE_MISMATCH:{str(e)}"}
    ...
    if lanes_file is None:
        ...
        file_payload = {"policy_tag":"projected(columns@k=3,file)",
                        "results":{"k3":{"eq": None}}, "na_reason_code":"FILE_PROJECTOR_MISSING"}
    elif len(lanes_file) != (_hard_co_shape(d3)[1]):
        freezer_payload = {"status":"NA", "na_reason_code":"FILE_PROJECTOR_WRONG_SIZE"}
        file_payload = {"policy_tag":"projected(columns@k=3,file)",
                        "results":{"k3":{"eq": None}}, "na_reason_code":"FILE_PROJECTOR_WRONG_SIZE"}
        ...
        freezer_payload = {"status":"OK", "na_reason_code": None}
        try:
            R3f = _hard_co_xor(_hard_co_mm2(H2,d3), _hard_co_xor(C3, _hard_co_eye(nC)))
            eqf = _hard_co_masked_allzero_cols(R3f, lanes_file)
            mismf = []
            mR,nR = _hard_co_shape(R3f)
            for j in range(nR):
                if lanes_file[j]==1:
                    for i in range(mR):
                        if R3f[i][j]==1:
                            mismf.append(j); break
            file_payload = {"policy_tag":"projected(columns@k=3,file)",
                            "results":{"k3":{"eq": bool(eqf)}, "selected_cols": list(lanes_file), "mismatch_cols_selected": mismf}}
        except Exception as e:
            file_payload = {"policy_tag":"projected(columns@k=3,file)",
                            "results":{"k3":{"eq": None}}, "na_reason_code": f"SHAPE_MISMATCH:{str(e)}"}
