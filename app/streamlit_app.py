...
with tab1:
    ...
    if st.button("Run Unit"):
        out = unit_check(boundaries, cmap, shapes)
        st.json(out)
...
with tab2:
    ...
    if st.button("Run Overlap"):
        if not d_H:
            ...
            out = overlap_check(boundaries, cmap, H)
            st.json(out)
...
with tab3:
    ...
    if triangle is None:
        ...
        if st.button("Run Triangle"):
            out = triangle_check(boundaries, triangle)
            st.json(out)
...
with tab4:
    ...
    if any(s not in ("I","C") for s in sched):
        ...
        if st.button("Run Tower & save CSV"):
            ...
            run_tower(sched, cmap, shapes, seed, csv_path, schedule_name="custom")
